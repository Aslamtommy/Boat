...
const placeOrder = async (req, res) => {
    try {
        ...
        // Retrieve cart data
        const orderProductsData = await cart.findOne({ userId: userId }).populate('items.productId');

        console.log('Retrieved cart data:', orderProductsData);

        // Retrieve address data
        const addressData = await address.findOne({ userid: userId });
        const selectedAddress = addressData.address.find(item => item._id == addressId);
...
const placeOrder2 = async (req, res) => {
    try {
        ...
        for (const item of orderProductsData.items) {
            ...
            if (newStock < 0) {
                return res.status(400).json({ success: false, message: `Insufficient stock for product with ID ${item.productId}` });
            }
...
const returnorder=async(req,res)=>{
    try {
        ...
        if (orderData.payMethod === 'Online Payment') {
        ...
        if (userWallet) {
            ...
            userWallet.transaction.push({ amount: orderData.total, reason: "return a product", transactionType: "credit" });
...
const cancelorder = async (req, res) => {
    try {
        ...
        if (userWallet) {
            ...
            userWallet.transaction.push({ amount: orderData.total, reason: "Cancelled a product", transactionType: "credit" });
...
const updatstatus = async (req, res) => {
    try {
        ...
        return res.json({ success: true, message: 'Order status updated successfully', updatedOrder: orderToUpdate });
    } catch (error) {
...
const saveInvoice = async (req, res) => {
    try {
        ...
        // Print Date
        doc.font('Helvetica-Bold').text(`Date: ${moment(orderData.date).format('LL')}`);
        ...
        orderData.items.forEach(item => {
            const productName = item.productId ? item.productId.name || 'Product Name Unavailable' : 'Product Name Unavailable';
        ...
        // Print Total Price
        doc.font('Helvetica-Bold').text(`Total Price: $${totalPrice.toFixed(2)}`).moveDown();

        // Print Thank you message
        doc.font('Helvetica-Bold').text('Thank you for your business!', { align: 'center' });
