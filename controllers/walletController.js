    ...
    walletData.forEach((walletItem) => {
      if (Array.isArray(walletItem.transaction)) {
        walletItem.transaction.forEach((transactionItem) => {
          ...
          transactionItem.date = moment(parsedDate).format(
            "YYYY-MM-DD HH:mm:ss"
          );
          console.log(
            `Formatted date for transaction: ${transactionItem.date}`
          );
        });
      } else if (typeof walletItem.transaction === "object") {
        const parsedDate = moment(walletItem.transaction.date).toDate();
        console.log(
          `Original date for transaction: ${walletItem.transaction.date}`
        );
        console.log(`Parsed date for transaction: ${parsedDate}`);
        walletItem.transaction.date = moment(parsedDate).format(
          "YYYY-MM-DD HH:mm:ss"
        );
        console.log(
          `Formatted date for transaction: ${walletItem.transaction.date}`
        );
      }
    });
